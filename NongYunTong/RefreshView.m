...
- (void)setupWithOwner:(UIScrollView *)owner_  delegate:(id)delegate_ {
    ...
    [owner insertSubview:self atIndex:0];//将RefreshView嵌入ScrollView上（）
...
- (void)stopLoading {
    // control
    isLoading = NO;//数据加载完毕，将该布尔变量置为NO，为下一次动作做准备
    //NSLog(@"停止加载动画。。。");
    // Animation
...
//    owner.contentInset = UIEdgeInsetsZero;
    ...
    self.refreshArrowImageView.transform = CGAffineTransformMakeRotation(0);//旋转下拉箭头
