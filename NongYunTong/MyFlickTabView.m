...
@interface MyFlickTabView (Private)
- (void)setupCaps;
@end


@implementation MyFlickTabView
...
- (void)awakeFromNib {
	...
	if(scrollView.subviews && scrollView.subviews.count > 0) {
		[(MyFlickTabButton*)[scrollView.subviews objectAtIndex:0] markSelected];
	}
...
- (void)reloadData {
	...
	for(x=0;x<items;x++) {
		...
		MyFlickTabButton* button = [[MyFlickTabButton alloc] initWithFrame:CGRectZero];
...
- (void)buttonClicked:(MyFlickTabButton*)button {
...
- (void)selectTabAtIndex:(NSInteger)index animated:(BOOL)animated {
	...
	[scrollView.subviews makeObjectsPerformSelector:@selector(markUnselected)];
	[(MyFlickTabButton*)[scrollView.subviews objectAtIndex:index] markSelected];
	
	CGRect rect = ((MyFlickTabButton*)[scrollView.subviews objectAtIndex:index]).frame;
...
- (NSInteger)selectedTabIndex {
	...
	for(MyFlickTabButton* tab in scrollView.subviews) {
		if([tab isMemberOfClass:[MyFlickTabButton class]]) {
