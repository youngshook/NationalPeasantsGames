...
@interface GCSearchSectionController ()
...
@implementation GCSearchSectionController
	...
	if ((self = [super init])) {
        if (![givenViewController respondsToSelector:@selector(tableView)]) {
            ...
            [NSException raise:@"Wrong view controller" 
                        format:@"The passed view controller to GCSearchSectionController must respond to the tableView proprety"];
        }
